...
async function captureSite(site) {
  ...
  const screenshotPath = path.join(IMAGE_DIR, `${site.slug}.png`);

  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    console.log(
      `[${site.name}] Capture attempt ${attempt}/${MAX_ATTEMPTS}`
    );
    ...
    try {
      ...
      // Allow the Flutter application, SolarEdge data and software-rendered
      // graphics time to initialise.
      await page.waitForTimeout(35_000);

      for (let check = 1; check <= 3; check++) {
        // Give Flutter two complete animation frames before taking the image.
        await page.evaluate(
          () =>
            new Promise((resolve) =>
        ...
        if (screenshot.length >= MIN_SCREENSHOT_SIZE) {
          await fs.writeFile(screenshotPath, screenshot);

          console.log(
            `[${site.name}] Saved ${screenshotPath} ` +
            `(${Math.round(screenshot.length / 1024)} KiB)`
          );
        ...
        if (check < 3) {
          console.log(
            `[${site.name}] Screenshot is still blank or incomplete. ` +
      ...
      console.log(
        `[${site.name}] Attempt ${attempt} error: ${error.message}`
      );
    } finally {
    ...
    if (attempt < MAX_ATTEMPTS) {
      console.log(
        `[${site.name}] Starting a fresh browser session before retrying.`
      );
  ...
  console.log(
    `[${site.name}] All capture attempts failed. Checking for the ` +
    `previously published screenshot.`
  );

  const restored = await restorePreviousScreenshot(site);
  ...
  const fallbackFilename = `${site.slug}-unavailable.svg`;
  const fallbackPath = path.join(IMAGE_DIR, fallbackFilename);
...
async function restorePreviousScreenshot(site) {
  ...
  const previousImageUrl =
    `https://${owner}.github.io/${repositoryName}/images/${site.slug}.png` +
    `?restore=${Date.now()}`;

  try {
    const response = await fetch(previousImageUrl, {
      headers: {
        "User-Agent": "SolarEdge-Signage-Capture",
    ...
    if (!response.ok) {
      console.log(
        `[${site.name}] Previous screenshot returned HTTP ` +
        `${response.status}.`
      );
      return false;
    }
    ...
    if (
      ...
      console.log(
        `[${site.name}] Previous screenshot was not a valid PNG.`
      );
      return false;
    }

    await fs.writeFile(
      path.join(IMAGE_DIR, `${site.slug}.png`),
      buffer
    );

    console.log(
      `[${site.name}] Restored the previously published screenshot.`
    );

    return true;
  } catch (error) {
    console.log(
      `[${site.name}] Could not restore previous screenshot: ` +
      `${error.message}`
    );
...
<svg xmlns="http://www.w3.org/2000/svg"
  ...
  <text x="960"
        ...
        font-size="58"
        font-weight="600">${safeName}</text>
  <text x="960"
        y="585"
        text-anchor="middle"
        fill="#b8c4ce"
        font-family="Arial, sans-serif"
        font-size="36">Dashboard temporarily unavailable</text>
</svg>`;
}
...
<body>
  ...
  <script>
    ...
    const imageUrl = ${JSON.stringify(result.image)};
    ...
    function refreshDashboard() {
      ...
      preload.src =
        imageUrl + "?refresh=" + new Date().getTime();
    }
...
try {
  for (const site of sites) {
    ...
    const pageDirectory = path.join(OUTPUT_DIR, site.slug);
...
const successfulCaptures = [...results.values()].filter(
  (result) => result.success
).length;

console.log(
  `Finished: ${successfulCaptures}/${sites.length} dashboards ` +
  `have a current or previously published screenshot.`
);

// Allow deployment to continue even if an individual SolarEdge dashboard failed.
process.exitCode = 0;
